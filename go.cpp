...
Go_state::Go_state(const Go_state &other)
    : state_(new GameState()), turn_(other.turn_), moveCounter_(other.moveCounter_), skipCounter_(other.skipCounter_), colorCaptures_(other.colorCaptures_)
{
	(*state_) = *(other.state_);
}
...
double Go_state::rollout() const
{
	...
	for (int i = 0; i < MAXSTEPS; i++)
	{
		...
		if (s.is_terminal())
		{
			auto scoreTerritory = s.state_->ScoreCurrentStateFinal();

			return (1.0 * scoreTerritory.first - scoreTerritory.second) / 81.0 + 0.5;
		}
	...
	return (1.0 * scoreTerritory.first - scoreTerritory.second) / 81.0 + 0.5;
	//if (scoreTerritory.first - colorCaptures_.first > ) {
	//	// Black won
	//	return 1;
	//}
